...
- `order.html` — (advanced) scale to target servings, convert to procurement units, illustrative pricing, and a simple pending→**approved** gate (re-scaling resets it). Reached per-recipe from `ingredients.html`, or from the index's Advanced nav with no `?id=`, in which case it shows a **recipe picker** (so both advanced pages are reachable from the index). Ambiguous lines — the `salt and pepper` split (shown as two lines with a caption) and any flagged line — get an editable grams input that live-reprices that line and the total in place (class `.qty-edit` + `data-idx`, updated without a full re-render so the input keeps focus). Don't reintroduce whitespace `id`s or per-keystroke re-render here — both were bugs.
